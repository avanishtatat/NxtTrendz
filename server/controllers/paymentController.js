...
const MAX_ORDER_AMOUNT_INR = 50000;

if (!process.env.RAZORPAY_KEY_ID || !process.env.RAZORPAY_KEY_SECRET) {
  throw new Error(
    "Razorpay API keys must be defined in environment variables.",
  );
}
...
export const createOrder = async (req, res) => {
  try {
    ...
    if (!cart || cart.items.length === 0) {
      return res
        .status(400)
        .json({ error: "Cart is empty. Cannot create order." });
    }
    ...
    // Validate total amount against maximum allowed limit for test mode
    if (totalAmount > MAX_ORDER_AMOUNT_INR) {
      return res
        .status(400)
        .json({
          error: `Order amount ${totalAmount} exceeds maximum allowed amount of INR ${MAX_ORDER_AMOUNT_INR}. Please remove some items from your cart.`,
        });
    }
  ...
  try {
    ...
    if (user.isPrime && user.primeExpiresAt > new Date()) {
      return res
        .status(400)
        .json({
          error: "You are already a prime member.",
          primeExpiresAt: user.primeExpiresAt,
        });
    }
    const options = {
      amount: PRIME_PRICE_INR * 100,
      currency: "INR",
      receipt: `prime_receipt_${Date.now()}`,
    };
    const primeOrder = await razorpayInstance.orders.create(options);
    return res
      .status(201)
      .json({
        razorpayOrderId: primeOrder.id,
        amount: primeOrder.amount,
        currency: primeOrder.currency,
      });
  } catch (error) {
    console.error("Error creating prime order:", error.message);
    return res
      .status(500)
      .json({ error: "An error occurred while creating the prime order." });
  }
};
...
export const verifyPrimePayment = async (req, res) => {
  ...
  try {
    const alreadyUsed = await User.findOne({
      "primePayment.razorpayOrderId": razorpayOrderId,
    });
    if (alreadyUsed) {
      return res
        .status(409)
        .json({ error: "This prime order has already been processed." });
    }
    const isValid = verifySignature(
      razorpayOrderId,
      razorpayPaymentId,
    ...
    return res
      .status(500)
      .json({ error: "An error occurred while verifying the prime payment." });
  }
};
